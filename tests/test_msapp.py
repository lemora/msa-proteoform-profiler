...
def test_msa_should_remove_empty_cols():
    ...
    mat = np.array([[0, 1, 1], [0, 1, 0], [0, 1, 1]], dtype=np.uint8)
    msa.init_from_mat(mat)
    msa.remove_empty_cols()

    should_be = np.array([[0, 1], [0, 0], [0, 1]], dtype=np.uint8)
...
def test_msa_should_not_remove_nonempty_cols():
    ...
    msa = MultiSeqAlignment()
    mat = np.array([[0, 0, 1], [0, 1, 0], [0, 1, 1]], dtype=np.uint8)
...
def test_msa_should_sort_rows_correctly():
    ...
    msa = MultiSeqAlignment()
    mat = np.array([[0, 1, 1], [1, 1, 1], [0, 1, 0]], dtype=np.uint8)
    msa.init_from_mat(mat)

    msa.sort_by_metric()
    should_be = np.array([[0, 1, 0], [0, 1, 1], [1, 1, 1]], dtype=np.uint8)
    assert_array_equal(msa._mat, should_be)
...
def test_msa_filter_by_full_row_should_be_noop():
    ...
    msa = MultiSeqAlignment()
    mat = np.array([[0, 1, 1], [1, 1, 1], [0, 0, 0]], dtype=np.uint8)
    should_be = copy.deepcopy(mat)
    msa.init_from_mat(mat)
...
def test_msa_should_filter_by_correct_idx():
    ...
    msa = MultiSeqAlignment()
    mat = np.array([[0, 1, 0], [1, 1, 1], [0, 1, 1]], dtype=np.uint8)
    msa.init_from_mat(mat)

    msa.filter_by_reference(0)
    should_be = np.array([[0, 0], [1, 1], [0, 1]], dtype=np.uint8)
    assert_array_equal(msa._mat, should_be)
